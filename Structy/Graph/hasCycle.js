// has cycle
// Write a function, hasCycle, that takes in an object representing the adjacency list of a directed graph. The function should return a boolean indicating whether or not the graph contains a cycle.

test_00:
hasCycle({
  a: ["b"],
  b: ["c"],
  c: ["a"],
}); // -> true
test_01:
hasCycle({
  a: ["b", "c"],
  b: ["c"],
  c: ["d"],
  d: [],
}); // -> false
test_02:
hasCycle({
  a: ["b", "c"],
  b: [],
  c: [],
  e: ["f"],
  f: ["e"],
}); // -> true
test_03:
hasCycle({
  q: ["r", "s"],
  r: ["t", "u"],
  s: [],
  t: [],
  u: [],
  v: ["w"],
  w: [],
  x: ["w"],
}); // -> false
test_04:
hasCycle({
  a: ["b"],
  b: ["c"],
  c: ["a"],
  g: [],
}); // -> true
test_05:
hasCycle({
  a: ["b"],
  b: ["c"],
  c: ["d"],
  d: ["b"],
}); // -> true

//n = number of nodes
// time O(n^2)
// space O(n)
const hasCycle = (graph) => {
    const visiting = new Set()
    const visited = new Set()
    for(let node in graph){
      if(traverseSegment(graph, node, visiting, visited) === true){
        return true
      }
    }
    return false
  };
  
  const traverseSegment = (graph, node, visiting, visited) => {
    if (visited.has(node)) return false
    if (visiting.has(node)) return true
    visiting.add(node)
    for(let neighbor of graph[node]){
      if(traverseSegment(graph, neighbor, visiting, visited) === true){
        return true
      }
    }
      visiting.delete(node)
      visited.add(node)
    return false
  }
  
  module.exports = {
    hasCycle,
  };
  