...
const breakingBoundaries = (m, n, k, r, c, memo = {}) => {
    //define inbounds for row
    //define inbounds for column
    //store positions in a memo
    //use recursion to store the count
    //make deltas to represent movement and loop through the deltas?
    //if you get out of bounds, return 1
    const key = k + ',' + r + ',' + c
    if(key in memo) return memo[key]
    const inboundRow = 0 <= r && r < m 
    const inboundCol = 0 <= c && c < n
    if(!inboundRow || !inboundCol) return 1
    if(k === 0) return 0
    let total = 0 
    const deltas = [
      [1, 0],
      ...
      for(let delta of deltas){
        const [rowChange, colChange] = delta
        total += breakingBoundaries(m, n, k - 1, rowChange + r, colChange + c, memo)
      }
    memo[key] = total
      return memo[key]
  };
