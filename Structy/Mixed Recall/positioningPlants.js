...
const positioningPlants = (costs, position = 0, prevPlant = null, memo = {}) => {
    //postion is represented by row
    //flower type represented by column
    const key = position + ',' + prevPlant
    if(key in memo) return memo[key]
    if (position === costs.length) return 0
    let min = Infinity
    for(let plantType = 0; plantType < costs[position].length; plantType++){
      if(plantType !== prevPlant){
        const candidate = costs[position][plantType] + positioningPlants(costs, position + 1, plantType, memo)
